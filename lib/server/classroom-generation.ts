...
function normalizeLanguage(language?: string): 'en-US' | 'hi-IN' {
...
export async function generateClassroom(
  ...
  if (input.enableWebSearch) {
    const firecrawlKey = resolveWebSearchApiKey();
    if (firecrawlKey) {
      try {
      ...
      log.warn('enableWebSearch is true but no Firecrawl API key configured, skipping web search');
    }
  }
