...
Kaksh AI generation does not automatically reuse the OpenClaw agent's current model or API key.

Kaksh AI server APIs resolve their own model and provider keys from Kaksh AI server-side config.

This skill does not rely on runtime overrides for model, provider, API key, base URL, or provider type.

If the user wants to change any of those, they must edit Kaksh AI server-side config files.
...
- Kaksh AI server fallback is currently `gpt-4o-mini` if `DEFAULT_MODEL` is unset.
...
Do not recommend bare model IDs such as `gemini-3-flash-preview` by themselves, because Kaksh AI will otherwise parse them as OpenAI models.
...
- "I recommend configuring Kaksh AI through `.env.local` first. Please edit that file locally and tell me when you're done."
...
|---------|-----------------|-------------|
| Web Search | `FIRECRAWL_API_KEY` | Enriches outlines with real-time web research |
| Image Generation | `IMAGE_NANO_BANANA_API_KEY` | Generates images for slides |
| Video Generation | `VIDEO_VEO_API_KEY`, `VIDEO_SORA_API_KEY` | Generates short videos (any one suffices) |
| TTS | `TTS_OPENAI_API_KEY`, `TTS_AZURE_API_KEY`, `TTS_ELEVENLABS_API_KEY` | Text-to-speech narration (any one suffices) |
